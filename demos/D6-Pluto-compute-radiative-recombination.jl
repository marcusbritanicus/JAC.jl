...
# ╔═╡ d6001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ d6001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ d6001003-0000-4000-8000-000000000003
md"""
# Compute radiative recombination cross sections
"""

# ╔═╡ d6001004-0000-4000-8000-000000000004
md"""
**Radiative recombination** (RR) is the time-reversed process of photoionization:

$$X^{q+} \;+\; e^- \;\longrightarrow\; X^{(q-1)+} \;+\; h\nu$$

An ion captures a free electron into a bound orbital while emitting a photon.  Because
it is the time-reverse of photo-ionisation, its cross section is connected to the
photo-ionisation cross section through the **Milne relation**.

JAC computes RR cross sections via `PhotoRecombination.Settings` passed as
`processSettings` in `Atomic.Computation`.  The key input is a list of
`electronEnergies` (in atomic units) at which the cross section is evaluated.

- `initialConfigs` specifies the **ion** before electron capture (the H-like target).
- `finalConfigs` specifies the **bound states** after recombination (the He-like product).
"""

# ╔═╡ d6001005-0000-4000-8000-000000000005
##  A grid of electron kinetic energies (in a.u.) for the cross-section scan
eEnergies = [0.01, 0.05, 0.10, 0.20, 0.50, 1.00, 2.00, 5.00]

##  PhotoRecombination settings: E1 in Coulomb gauge, print line list before evaluation
rrSettings = PhotoRecombination.Settings(PhotoRecombination.Settings();
                 multipoles       = [E1],
                 gauges           = [UseCoulomb],
                 electronEnergies = eEnergies,
                 printBefore      = true)

# ╔═╡ d6001006-0000-4000-8000-000000000006
md"""
## Radiative recombination into helium-like oxygen (Z=8)

We compute RR of H-like O⁷⁺ (one electron, `1s` ground state) capturing an electron
into the lowest He-like O⁶⁺ states (`1s²`, `1s 2s`, `1s 2p`).
"""

# ╔═╡ d6001007-0000-4000-8000-000000000007
##  RR into He-like O (Z=8): H-like ground + e⁻ → He-like bound states
wa_O = Atomic.Computation(Atomic.Computation();
           name             = "Radiative recombination: H-like → He-like O (Z=8)",
           ...
           finalConfigs     = [Configuration("1s^2"),
                               Configuration("1s 2s"),
                               Configuration("1s 2p")],
           processSettings  = rrSettings)
perform(wa_O)

# ╔═╡ d6001008-0000-4000-8000-000000000008
md"""
## Radiative recombination into helium-like neon (Z=10)

Repeating the calculation for Ne⁹⁺ → Ne⁸⁺ allows comparison of how the RR cross
section scales with $Z$.  We reuse the same settings and electron-energy grid.
"""

# ╔═╡ d6001009-0000-4000-8000-000000000009
##  RR into He-like Ne (Z=10): reuse settings, change nuclear model only
wa_Ne = Atomic.Computation(wa_O;
            name         = "Radiative recombination: H-like → He-like Ne (Z=10)",
            nuclearModel = Nuclear.Model(10.))
perform(wa_Ne)

# ╔═╡ d6001010-0000-4000-8000-000000000010
md"""
## Radiative recombination into helium-like argon (Z=18)

For Ar¹⁷⁺ → Ar¹⁶⁺ the inner-shell orbitals are more tightly bound and the RR cross
section is larger at low energies relative to higher-$Z$ ions.
"""

# ╔═╡ d6001011-0000-4000-8000-000000000011
##  RR into He-like Ar (Z=18)
wa_Ar = Atomic.Computation(wa_O;
            name         = "Radiative recombination: H-like → He-like Ar (Z=18)",
            nuclearModel = Nuclear.Model(18.))
perform(wa_Ar)

# ╔═╡ d6001012-0000-4000-8000-000000000012
md"""
## Summary

The three RR calculations illustrate:

1. `PhotoRecombination.Settings(PhotoRecombination.Settings(); electronEnergies=..., ...)` 
   sets up the energy grid and controls which multipoles and gauges are included.
2. `initialConfigs` is the ion state *before* recombination; `finalConfigs` lists the
   bound states *after* capture.
3. The keyword copy-constructor `Atomic.Computation(wa_O; nuclearModel=...)` makes
   it trivial to scan over different elements.
4. Cross sections generally increase with $Z$ at fixed electron energy because the
   Coulomb attraction grows and the bound-state wave functions are more compact.

For a more refined calculation one would include higher partial waves (`maxKappa > 0`)
and additional final-state configurations (e.g.\ `1s 3s`, `1s 3p`, `1s 3d`).
"""
