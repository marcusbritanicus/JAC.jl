...
# ╔═╡ b6001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ b6001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ b6001003-0000-4000-8000-000000000003
md"""
# Expand atomic levels into a LS-coupled basis
"""

# ╔═╡ b6001004-0000-4000-8000-000000000004
md"""

In atomic spectroscopy, levels are frequently classified in the $LSJ$ notation by means of the spectroscopic term symbols $^{2S+1}L_J$. Since modern relativistic structure codes — including JenaAtomicCalculator — work in a $jj$-coupled basis, a **unitary transformation** from $jj$J $\to$ $LS$J coupling is needed to produce these familiar labels.

This transformation critically depends on the shell structure of the atom or ion and is provided by the `LSjj` module in JenaAtomicCalculator. At present, the transformation is restricted to **atoms with not more than two open non-relativistic subshells**. The control parameter is `jjLS::LSjjSettings` inside `AsfSettings`.

"""

# ╔═╡ b6001005-0000-4000-8000-000000000005
? AsfSettings

# ╔═╡ b6001006-0000-4000-8000-000000000006
md"""

We enable the $jj \to LS$ transformation by constructing an `AsfSettings` with `jjLS=LSjjSettings(true)`. All other SCF parameters are kept at their defaults.

"""

# ╔═╡ b6001007-0000-4000-8000-000000000007
asfSettings = AsfSettings(AsfSettings(); generateScf=true, maxIterationsScf=20, jjLS=LSjjSettings(true))

# ╔═╡ b6001008-0000-4000-8000-000000000008
md"""

## Beryllium ($Z = 4$): $\;2s^2 + 2s\,2p$ configurations

We start with neutral beryllium and the two lowest configurations $[He]\,2s^2$ and $[He]\,2s\,2p$. This includes the $^1P_1$–$^3P_1$ levels that mix under $jj$ coupling.

"""

# ╔═╡ b6001009-0000-4000-8000-000000000009
begin
wa = Atomic.Computation(Atomic.Computation();
                         name         = "jj-LS transformation: Be isoelectronic",
                         grid         = Radial.Grid(true),
                         nuclearModel = Nuclear.Model(4.),
                         configs      = [Configuration("[He] 2s^2"), Configuration("[He] 2s 2p")],
                         asfSettings  = asfSettings)
perform(wa)
end

# ╔═╡ b6001010-0000-4000-8000-000000000010
md"""

Apart from the SCF convergence, the output shows the $LS$ expansion of all levels. For neutral beryllium, the $2s^2\;^1S_0$ and the $2s\,2p\;^3P_{0,2}$ levels are essentially *pure* states; only the two $^{1,3}P_1$ levels can mix. Because the $jj$ mixing is negligibly small for Be, the $LS$ compositions appear virtually 100 % pure.

## Beryllium-like argon ($Z = 18$) and xenon ($Z = 54$)

We repeat the computation for heavier elements simply by modifying the nuclear model in the copy constructor. The spin-orbit interaction grows with $Z$, so the $jj$ mixing of the $^{1,3}P_1$ pair increases:

"""

# ╔═╡ b6001011-0000-4000-8000-000000000011
perform(Atomic.Computation(wa; nuclearModel=Nuclear.Model(18.)))

# ╔═╡ b6001012-0000-4000-8000-000000000012
perform(Atomic.Computation(wa; nuclearModel=Nuclear.Model(54.)))

# ╔═╡ b6001013-0000-4000-8000-000000000013
md"""

Comparing the three outputs, the $^3P_1$ weight of the lower $^{3}P_1$ level drops from 1.00 ($Z=4$) to about 0.85 ($Z=18$) to 0.76 ($Z=54$). This mixing is precisely the reason why the intercombination line $2s\,2p\;^{3}P_1 - 2s^2\;^1S_0$ is much weaker than the resonance line $2s\,2p\;^{1}P_1 - 2s^2\;^1S_0$ in neutral beryllium but becomes comparable to it for medium and heavy elements.

To inspect the default threshold parameters of the transformation, use:

"""

# ╔═╡ b6001014-0000-4000-8000-000000000014
? LSjjSettings

# ╔═╡ b6001015-0000-4000-8000-000000000015
md"""

## Iron ($Z = 26$): $\;3d^3$ ground configuration

The $jj \to LS$ transformation becomes increasingly complex for open $d$- and $f$-shells. As an example we compute the levels of the $[Ar]\,3d^3$ ground configuration of iron-like $^{56}$Fe:

"""

# ╔═╡ b6001016-0000-4000-8000-000000000016
begin
wb = Atomic.Computation(Atomic.Computation();
                         name         = "jj-LS transformation: Fe 3d³",
                         grid         = Radial.Grid(true),
                         nuclearModel = Nuclear.Model(26.),
                         configs      = [Configuration("[Ar] 3d^3")],
                         asfSettings  = asfSettings)
perform(wb)
end

# ╔═╡ b6001017-0000-4000-8000-000000000017
md"""

There are 19 levels arising from the single $3d^3$ configuration. While for most levels one $LS$ term strongly dominates, sizeable mixing appears in several cases — see, for example, the adjacent levels near the middle of the list. To illustrate how much further the complexity can grow, we examine a potassium-like iron configuration with a $2p$ hole:

"""

# ╔═╡ b6001018-0000-4000-8000-000000000018
perform(Atomic.Computation(wb; configs=[Configuration("[Ne] 3s^2 3p^5 3d^2")]))

# ╔═╡ b6001019-0000-4000-8000-000000000019
md"""

This produces 45 fine-structure levels. Their $LS$ decompositions show that the coupling character becomes highly mixed for many levels, making spectroscopic identification from energies alone increasingly difficult. The `LSjj` transformation provides a systematic way to expose the dominant character of each level and to assign approximate quantum numbers.
