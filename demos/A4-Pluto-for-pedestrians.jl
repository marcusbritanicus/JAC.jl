...
md"""
Let us first invoke JAC simply by **using JenaAtomicCalculator**
"""

# ╔═╡ 956d7bd6-d59f-4daa-88b6-9b8879918d79
md"""
Most users, likely, wish to see *first, tangible results* without going through lengthy notebooks and instructions. In JAC, therefore, we support **simple atomic comptutation for pedestrians**. These computations require a minimum input but are made with strong assumptions in mind, which might not fit the requests by the user. The computations in this notebook may server as a *first appetizer*, whereas further details will be explained in subsequent notebooks.

The same approach might be **useful for occasional use of JAC**, if comptuational details and accuracy is not considered so relevant.
"""

# ╔═╡ 5e49b1e1-23ca-426b-915d-86a276e88c72
md""""
Perhaps, the most frequent computations refer to the ground and low-lying excitations energies as they can be obtained (with much better accuracy) from the NIST Atomic Level Database. If, however, these data are not available, you call `computeLevelEergies` by just providing the nuclear charge and the configurations of interest.

For the **low-lying levels of atomic neon (Z=10)**, for example, all what you need to provide is:
"""

# ╔═╡ 83e1c39e-35a0-4357-a900-14b07e2724bc
begin
...
md"""
Apart from level energies, we can compute also approximate *radiative* and *non-radiative (Auger)* rates for transitions between the levels from some initial- and final-state configurations. If, for example, we consider the **photoemission rates of 1s ionized neon** atoms, we can simple write:
"""

# ╔═╡ cbc61e3a-b650-4577-bf18-a241b0879c02
md"""
Here, of course, we could omit the first line since the nuclear charge was set before. Obviously, the given configurations must support a simple photon emission, i.e. must have the same number of electrons and should allow for a release of (photon) energy.

A rather analogue input is sufficient, to compute the **Auger rates for a 1s ionized neon** atom:
"""

# ╔═╡ 8386d7ea-8ca4-43a1-a9a7-051c3dbf17c3


# ╔═╡ c1b8bdb4-f0a2-49f6-9bbc-7ac7ed7efb10
md"""
Along similar lines, we can compute **photoionization cross sections, again for neutral neon**:
"""
...
# ╔═╡ 8853a76a-7485-443e-b7a7-6d52de3bf555
begin
	setDefaults("nuclear: charge", 10.0)
