...
md"""
# Hydrogenic ions:$\;\:$ Compute and deal with hydrogenic solutions
"""

# ╔═╡ a63c4f19-ef00-4ce0-9383-1889c6aa70cd
md"""
In this notebook, we wish to collect and demonstrate some simple computations for hydrogen and hydrogenic ions as the might be discussed in textbooks. In particular, we show how readily the well-known solutions can be **obtained and utilized** for explaining the basic behavior of atoms or to make quick estimates.
"""

# ╔═╡ 75061d53-4fb7-45a6-b8bb-c199d52479da
md"""
Let us first invoke JAC by **using JenaAtomicCalculator**
"""
...
Of course, we could plot the two functions `Pnr_1s` and `Pnr_2p` directly by some proper call of Plots, and if we give the correct arrays of radial mesh points from above.
"""

# ╔═╡ c0692594-1d25-43a5-96e8-902322273382


# ╔═╡ bcab2290-7783-4e79-9a22-35c7a191eec8
md"""
...
# ╔═╡ 550ca11f-27ad-4192-a1e6-4046193c3543


# ╔═╡ 3fd166e9-e638-43d6-9521-b52f114dd316
md"""
...
# ╔═╡ cba723cd-91dc-415b-8a0f-b4c6122b6e37


# ╔═╡ ef0fc41d-79c8-4d3c-9ee0-0fbb48b2ebf2
md"""
This `Basics.plot` function (from the Basics module) accepts for instance a list of (relativistic) radial `Orbitals`:
"""

# ╔═╡ f7c05b26-f395-4976-aeb3-49fcbd7e3af9


# ╔═╡ ece5e468-7e68-438a-adb5-e566fa6caf5d
md"""
...
# ╔═╡ b101aaf1-b5ba-4530-b7a3-beebae2ad79e


# ╔═╡ 379132eb-ccf7-4951-a68a-f9219f3992ab
md"""
...
# ╔═╡ 495a6478-c614-4c62-ac74-68a00a5ad922


# ╔═╡ a148f133-ff04-4850-9a85-6e087185cf28


# ╔═╡ 1487bd10-f375-4100-856d-f03b78eb798a
md"""
...
# ╔═╡ 0e45f529-7e21-4c91-b7a8-b66cf4bce075


# ╔═╡ f90a81cb-f4be-4cec-b56a-c31ce3fa462c
md"""
...
# ╔═╡ 5e765afe-5cf8-49ab-b301-96e5a8beaaa2
md"""
...
# ╔═╡ c48b391f-0a8c-4a93-a51f-24768ad91d0b


# ╔═╡ bfbbbc47-150a-44ba-a974-a51729a2c64a
md"""
...
# ╔═╡ 8398e267-058f-4951-8ca8-e45139648893


# ╔═╡ d99d94c0-e631-4a8a-a63c-00d44102a2cd
md"""
...
# ╔═╡ 3c733f60-c436-4382-8e5e-99ff4244f4c9


# ╔═╡ 689e1a8e-7e0c-4650-98cf-e99f7d5cd7d7
...
# ╔═╡ abcad25a-b912-42cc-831c-9f20101c499b
md"""
The radial dependence of the wave function is as expected, 
but we should still test it and check whether the generated wave functions 
are normalised, i.e. $\langle 1s|1s \rangle = 1$. 

To do that, we can use the `Trapz` integration package, which simply performs 
trapezoidal integration over Julia arrays. If you do not have this package installed, 
you can add it via `import Pkg; Pkg.add("Trapz")`.
"""
...
md"""
Now check normalisation \(\langle 1s|1s\rangle = 1\). We use trapezoidal integration (`Trapz.jl`).

Now, the normalisation can be calculated simply by integrating the square of the wave function on the grid it was generated on. Note that for multiplying vectors, one needs to use `.*` instead of `*`.  

We see that the normalisation is equal to unity, i.e. $\langle 1s|1s \rangle = 1$, as expected.
"""

# ╔═╡ 5433c5b9-d3dc-4454-8123-fcf4b5bf3cad


# ╔═╡ 08492a69-0f0c-4008-9d7e-a0ae1e81f03a
md"""
Now, the normalisation can be calculated simply by integrating 
the square of the wave function on the grid it was generated on. 
Note that for multiplying vectors, one needs to use `.*` instead of `*`.  

We see that the normalisation is equal to unity, i.e. 
$\langle 1s|1s \rangle = 1$, as expected.
"""

# ╔═╡ 01ee5969-6f33-445d-b403-8715fa38f252


# ╔═╡ 07187d5c-09f5-4498-86b6-7a1da50a0757
md"""
Next compute expectation values for hydrogen (in atomic units, \(a_0 = 1\)):
\(\langle 1/r \rangle\), \(\langle r \rangle\), and \(\langle r^2 \rangle\).

Further, we can verify other known properties of the hydrogen wave function.  
For example, the expectation values of the $1/r$, $r$, and $r^2$ operators 
are analytically known.  

As it is often taught in many university atomic physics courses:
- $\langle 1s|1/r|1s \rangle = 1 \, a_0$  
- $\langle 1s|r|1s \rangle = \tfrac{3}{2} \, a_0$  
- $\langle 1s|r^2|1s \rangle = 3 \, a_0$  
where $a_0$ is the Bohr radius.  

In JenaAtomicCalculator, all computations are internally performed 
and returned in **atomic units**. Hence $a_0 = 1$.  
We can, therefore, see that our calculation confirms the 
analytically predicted expectation values.
"""

# ╔═╡ 0cc59bc9-5ae8-4955-a809-3f86c1d446e6


# ╔═╡ c43f1262-724e-4ab2-9c0c-aba47f37d6fe
md"""
Further, we can verify other known properties of the hydrogen wave function.  
For example, the expectation values of the $1/r$, $r$, and $r^2$ operators 
are analytically known.  
...
# ╔═╡ 6149967a-10b5-4dde-9c54-55818a64f9d5


# ╔═╡ 64397f5a-b5bd-43eb-a66f-561860db2fab
md"""
Now compare nonrelativistic vs relativistic wavefunctions for a heavy ion (hydrogen-like uranium).

Until now, we considered the solutions of the nonrelativistic ion.  
In many cases, these nonrelativistic predictions are accurate.  

For heavy elements, however, the nonrelativistic theory breaks down, 
and the relativistic Dirac equation needs to be used instead.  

In the following examples, we will compare the nonrelativistic and 
relativistic radial wave functions and energies in order to get 
an intuition about the limits of the Schrödinger equation solutions.  

Firstly, let us generate the relativistic electron wavefunction for 
the hydrogen atom in its ground state, as well as the nonrelativistic 
and relativistic wave functions for hydrogen-like uranium.
"""

# ╔═╡ cdcb1b58-c8bd-4361-b071-ec72e7b13840


# ╔═╡ 9f8d8baf-72dd-4510-9698-1c1a4f50f7e0
md"""
As before, it is instructive to check that the generated wave functions are normalised. Then, let us plot the uranium radial wave functions.  

Note that the relativistic wave function is an array, containing two arrays — 
the large ($P$) and small ($Q$) components.  

The normalisation of a relativistic wavefunction is then given by:  
$\langle P|P\rangle + \langle Q|Q\rangle = 1$.
"""

# ╔═╡ c83ee411-5bb6-4ded-8d74-146c9b864395


# ╔═╡ 396547a2-35f3-4a57-b451-60517e026f64
md"""
Compute cumulative probability \(P_c(0,d)=\int_0^d |\psi(r)|^2\,dr\) and compare NR vs relativistic.

For example, we can calculate a cumulative probability, 
which expresses the probability to find the electron anywhere 
from the nucleus to a given distance $d$.  

This probability is written as:  
$P_c(0,d) = \int_0^d |P(r)|^2 \, dr$.  

Let us calculate and plot this cumulative probability for hydrogen-like uranium.
"""

# ╔═╡ 21e774ba-2be4-4bd7-8bfb-4ee46d1cf6db


# ╔═╡ c8c5aabd-d042-43bb-a77c-88a7b2071f3a
md"""
For example, we can calculate a cumulative probability, which expresses the probability to find the electron anywhere from the nucleus to a given distance $d$.  

This probability is written as: $P_c(0,d) = \int_0^d |P(r)|^2 \, dr$.  

Let us calculate and plot this cumulative probability for hydrogen-like uranium.
"""

# ╔═╡ 9f67bdae-da54-4278-9d26-0a23fc8f792b


# ╔═╡ 9907a947-70ab-4934-97d7-f3c9b4de9f12
md"""
Compare \(\langle r \rangle\) for hydrogen and uranium (NR vs relativistic).

Even here, we see that the relativistic calculation predicts 
the electron is found much nearer to the nucleus than 
the nonrelativistic solution.  

This shift can also be observed in the expectation values $\langle r \rangle$.  

- For hydrogen, $\langle r \rangle = 3/2$ in the nonrelativistic theory.  
- The relativistic correction shifts this by only $0.0018\%$ — negligible.  
- For uranium, the shift is **more than 20%**, which is significant.
"""

# ╔═╡ 3fb6128b-b413-43c5-8fe1-c005d9d9b4ba


# ╔═╡ f9b24de2-aa09-491a-8e48-b0c16522a746
md"""
Even here, we see that the relativistic calculation predicts 
the electron is found much nearer to the nucleus than 
the nonrelativistic solution.  
...
# ╔═╡ e55b43dc-e5d1-4153-8b8b-adde004588f8


# ╔═╡ d79afe30-78b8-4cbe-b3e9-e6d06356da90
md"""
Binding energies for H and U (NR vs relativistic).

This relativistic contraction strongly influences observables.  
	
For uranium, it shifts $\langle r \rangle$ significantly.  
This effect also shows up in **binding energies**.  

Let us calculate the binding energies for hydrogen and uranium 
(nonrelativistic vs relativistic).
"""

# ╔═╡ f985105f-1118-4d36-a124-a8aa3052ab2a


# ╔═╡ f18e1840-bcf3-428a-a532-ff1d64415a9b
md"""
This relativistic contraction strongly influences observables.  
...
# ╔═╡ b6b2b126-3281-4bc6-8aac-a901d23ea597


# ╔═╡ 759b7788-e0e5-41b6-95fa-0cc7e6b563c9
md"""
Binding energies vs nuclear charge (Z = 1…92): NR and relativistic, plus percent shift.

As expected, relativistic effects in hydrogen are tiny, 
while they are significant for uranium.  

To build intuition, it is useful to see the influence 
of relativistic effects for **all hydrogen-like ions** 
with nuclear charges $Z=1 \dots 92$.
"""

# ╔═╡ 18decce8-b266-4d11-ae3d-bc8512776eef
...
manifest_format = "2.0"
project_hash = "ff5a8d8af286cefeeb35dc954f215457035ec528"
