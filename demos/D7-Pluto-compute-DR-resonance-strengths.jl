...
# ╔═╡ d7001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ d7001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ d7001003-0000-4000-8000-000000000003
md"""
# Compute dielectronic recombination resonance strengths
"""

# ╔═╡ d7001004-0000-4000-8000-000000000004
md"""
**Dielectronic recombination** (DR) is a two-step resonant process:

$$X^{q+} \;+\; e^- \;\longrightarrow\; [X^{(q-1)+}]^{**} \;\longrightarrow\; X^{(q-1)+} \;+\; h\nu$$

1. The free electron is captured while simultaneously exciting a core electron
   (dielectronic capture into a doubly-excited **intermediate** state).
2. The doubly-excited intermediate state stabilises radiatively, emitting a photon
   and leaving the ion in a singly-excited or ground **final** state.

DR is the dominant recombination mechanism in hot, low-density plasmas (stellar
coronae, fusion devices) for ions with partially-filled shells.

In JAC the process requires three sets of configurations in `Atomic.Computation`:

| field | role |
|-------|------|
| `initialConfigs`       | the **ion** ground state before capture |
| `intermediateConfigs`  | the doubly-excited **resonance** states |
| `finalConfigs`         | the singly-excited states after radiative stabilisation |

Rate coefficients at chosen plasma temperatures are obtained by setting
`calcRateAlpha=true` and listing `temperatures` in `DielectronicRecombination.Settings`.
"""

# ╔═╡ d7001005-0000-4000-8000-000000000005
##  DR settings: E1 stabilisation, Coulomb gauge, compute rate coefficient at kT = 10-1000 eV
drSettings = DielectronicRecombination.Settings(DielectronicRecombination.Settings();
                 multipoles    = [E1],
                 gauges        = [UseCoulomb],
                 calcRateAlpha = true,
                 printBefore   = true,
                 temperatures  = [1.0e6, 1.0e7, 1.0e8])

# ╔═╡ d7001006-0000-4000-8000-000000000006
md"""
## DR resonance strengths for Li-like Ar (Z = 18)

**Li-like argon** (Ar¹⁵⁺, 3 electrons) is a benchmark DR system studied in storage-ring
experiments.  Here the L-shell DR channels are:

- **initial**: `1s² 2s` (Li-like ground)
- **intermediate**: `1s² 2s 2p` and `1s² 2p²` (Be-like doubly excited, $\Delta n = 0$ core excitation)
- **final**: `1s² 2s²` and `1s² 2s 2p` (Li-like after stabilisation)
"""

# ╔═╡ d7001007-0000-4000-8000-000000000007
##  DR for Li-like Ar (Z=18), L-shell channels
wa_Ar = Atomic.Computation(Atomic.Computation();
            name                   = "DR resonance strengths: Li-like Ar (Z=18)",
            grid                   = Radial.Grid(true),
            nuclearModel           = Nuclear.Model(18.),
            initialConfigs         = [Configuration("1s^2 2s")],
            intermediateConfigs    = [Configuration("1s^2 2s 2p"),
                                      Configuration("1s^2 2p^2")],
            finalConfigs           = [Configuration("1s^2 2s^2"),
                                      Configuration("1s^2 2s 2p")],
            processSettings        = drSettings)
perform(wa_Ar)

# ╔═╡ d7001008-0000-4000-8000-000000000008
md"""
## DR resonance strengths for Li-like Kr (Z = 36)

Krypton (Z=36) has a much stronger nuclear attraction, which shifts the resonance
energies upward and changes the relative importance of individual pathways.  The same
L-shell DR channels apply.
"""

# ╔═╡ d7001009-0000-4000-8000-000000000009
##  DR for Li-like Kr (Z=36): reuse all settings, change nuclear model only
wa_Kr = Atomic.Computation(wa_Ar;
            name         = "DR resonance strengths: Li-like Kr (Z=36)",
            nuclearModel = Nuclear.Model(36.))
perform(wa_Kr)

# ╔═╡ d7001010-0000-4000-8000-000000000010
md"""
## K-shell DR channels for Li-like Ar

For **KLL** DR the core excitation involves a $1s$ electron jumping to the $2p$
shell while the free electron is captured into $2p$:

- **intermediate**: `1s 2s^2 2p` and `1s 2s 2p^2` (triply-occupied L-shell)
- **final**: `1s^2 2s^2`, `1s^2 2s 2p` (after K-LL radiative stabilisation)

These K-shell resonances occur at much higher electron energies (keV range).
"""

# ╔═╡ d7001011-0000-4000-8000-000000000011
##  KLL DR channels for Li-like Ar (Z=18)
wa_Ar_K = Atomic.Computation(Atomic.Computation();
              name                = "KLL DR: Li-like Ar (Z=18)",
              grid                = Radial.Grid(true),
              nuclearModel        = Nuclear.Model(18.),
              initialConfigs      = [Configuration("1s^2 2s")],
              intermediateConfigs = [Configuration("1s 2s^2 2p"),
                                     Configuration("1s 2s 2p^2")],
              finalConfigs        = [Configuration("1s^2 2s^2"),
                                     Configuration("1s^2 2s 2p")],
              processSettings     = drSettings)
perform(wa_Ar_K)

# ╔═╡ d7001012-0000-4000-8000-000000000012
md"""
## Summary

These DR calculations illustrate:

1. Three distinct configuration lists are needed:
   `initialConfigs`, `intermediateConfigs`, and `finalConfigs`.
2. `DielectronicRecombination.Settings` controls which radiative multipoles stabilise
   the resonance, which gauge is used, and whether plasma rate coefficients are computed.
3. Setting `calcRateAlpha=true` and providing a `temperatures` list yields Maxwellian
   rate coefficients $\alpha_{\rm DR}(T)$ directly from the resonance strengths.
4. L-shell ($\Delta n = 0$) and K-shell (KLL) DR channels involve very different
   resonance energies and must be treated as separate computations.
"""
