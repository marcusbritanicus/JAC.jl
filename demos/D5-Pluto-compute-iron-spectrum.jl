...
# ╔═╡ d5001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ d5001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ d5001003-0000-4000-8000-000000000003
md"""
# Compute a photo-emission spectrum of Fe X
"""

# ╔═╡ d5001004-0000-4000-8000-000000000004
md"""
**Fe X** is singly-charged iron missing nine electrons: $Z = 26$, 17 electrons in a
Cl-like ground configuration $[\text{Ne}]\,3s^2 3p^5$.  Its UV and EUV emission lines
are prominent in solar corona spectra.

Here we compute electric-dipole (E1) and magnetic-dipole (M1) transition rates from
low-lying excited Fe X levels down to the ground state.  JAC uses
`PhotoEmission.Settings` inside an `Atomic.Computation` together with
`initialConfigs` (upper levels) and `finalConfigs` (lower levels).

Two gauge forms — **Coulomb** and **Babushkin** — are included as a self-consistency
check: agreement between the two indicates well-converged radial orbitals.
"""

# ╔═╡ d5001005-0000-4000-8000-000000000005
##  Radiative-transition settings: E1 + M1, both gauges, print line list
pSettings = PhotoEmission.Settings(PhotoEmission.Settings();
                multipoles  = [E1, M1],
                gauges      = [UseCoulomb, UseBabushkin],
                printBefore = true)

# ╔═╡ d5001006-0000-4000-8000-000000000006
md"""
## Excited and ground configurations of Fe X

The three lowest excited configurations above the ground state `[Ne] 3s² 3p⁵` are:
- `[Ne] 3s 3p⁶`  (one 3s electron promoted to complete the 3p shell)
- `[Ne] 3s² 3p⁴ 3d`  (one 3p electron promoted to 3d)

These provide the initial (upper) levels; the ground configuration provides the final
(lower) levels.
"""

# ╔═╡ d5001007-0000-4000-8000-000000000007
##  Fe X photo-emission spectrum: [Ne] 3s 3p^6 and [Ne] 3s^2 3p^4 3d → [Ne] 3s^2 3p^5
wa = Atomic.Computation(Atomic.Computation();
         name             = "Photo-emission spectrum: Fe X",
         grid             = Radial.Grid(true),
         nuclearModel     = Nuclear.Model(26.),
         initialConfigs   = [Configuration("[Ne] 3s 3p^6"),
                             Configuration("[Ne] 3s^2 3p^4 3d")],
         finalConfigs     = [Configuration("[Ne] 3s^2 3p^5")],
         processSettings  = pSettings)
perform(wa)

# ╔═╡ d5001008-0000-4000-8000-000000000008
md"""
## Adding the E2 multipole

Electric-quadrupole (E2) transitions become relevant for forbidden lines that dominate
in low-density astrophysical plasmas.  We extend the multipole list to `[E1, M1, E2]`.
"""

# ╔═╡ d5001009-0000-4000-8000-000000000009
##  Extend to E2 as well
pSettings_E2 = PhotoEmission.Settings(pSettings; multipoles=[E1, M1, E2])

wb = Atomic.Computation(wa;
         name            = "Photo-emission spectrum (E1+M1+E2): Fe X",
         processSettings = pSettings_E2)
perform(wb)

# ╔═╡ d5001010-0000-4000-8000-000000000010
md"""
## Fe XIV: a second coronal ion

Fe XIV (Z=26, 13 electrons, Al-like: $[\text{Ne}]\,3s^2 3p$) has its famous green
coronal line near 530 nm.  We repeat the computation for this ion using the
keyword copy-constructor to swap only the nuclear model and configurations.
"""

# ╔═╡ d5001011-0000-4000-8000-000000000011
##  Fe XIV (Al-like): [Ne] 3s 3p^2 → [Ne] 3s^2 3p
wc = Atomic.Computation(Atomic.Computation();
         name             = "Photo-emission spectrum: Fe XIV",
         grid             = Radial.Grid(true),
         nuclearModel     = Nuclear.Model(26.),
         initialConfigs   = [Configuration("[Ne] 3s 3p^2"),
                             Configuration("[Ne] 3p^3")],
         finalConfigs     = [Configuration("[Ne] 3s^2 3p")],
         processSettings  = pSettings)
perform(wc)

# ╔═╡ d5001012-0000-4000-8000-000000000012
md"""
## Summary

The three calculations demonstrate the standard workflow for photoemission spectroscopy
in JAC:

1. Choose `initialConfigs` (excited levels) and `finalConfigs` (lower levels) in
   `Atomic.Computation`.
2. Pass a `PhotoEmission.Settings` object via `processSettings`.
3. Including multiple gauges (Coulomb and Babushkin) provides a gauge-invariance
   check: closely matching values indicate well-converged SCF orbitals.
4. The keyword copy-constructor `Atomic.Computation(wa; ...)` makes it straightforward
   to vary the ion or the multipole selection.
"""
