...
# ╔═╡ c3001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ c3001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ c3001003-0000-4000-8000-000000000003
md"""
# Green function expansion for two-photon decay
"""

# ╔═╡ c3001004-0000-4000-8000-000000000004
md"""

Green functions appear in many branches of theoretical physics as solutions of inhomogeneous boundary-value problems. In atomic structure theory, an **approximate many-electron Green function** provides a compact representation of a sum over intermediate states and is therefore an essential ingredient for computing **second- and higher-order processes** such as two-photon decay, Rayleigh scattering, or stimulated Raman transitions.

In JenaAtomicCalculator, a `GreenExpansion` groups intermediate levels into **channels**, each defined by a total symmetry $J^\pi$. The expansion is controlled by the choice of an `AbstractGreenApproach` (which determines how the electron–electron interaction is handled in each channel) and an `AbstractExcitationScheme` (which determines which configurations enter the sum).

"""

# ╔═╡ c3001005-0000-4000-8000-000000000005
? AtomicState.GreenExpansion

# ╔═╡ c3001006-0000-4000-8000-000000000006
? AtomicState.AbstractGreenApproach

# ╔═╡ c3001007-0000-4000-8000-000000000007
md"""

The `GreenSettings` allow fine-grained control over which virtual orbitals enter the expansion:

"""

# ╔═╡ c3001008-0000-4000-8000-000000000008
? AtomicState.GreenSettings

# ╔═╡ c3001009-0000-4000-8000-000000000009
md"""

## Green expansion for the two-photon decay of beryllium-like xenon

As an application, we generate a Green function expansion suitable for estimating the E1-M1 two-photon decay rate of the metastable $1s^2 2s\,2p\;^3P_0$ level of beryllium-like xenon ($Z = 54$). The intermediate $J^\pi = 1^+$ and $J^\pi = 1^-$ channels are needed for the E1-M1 matrix element. We use the `SingleCSFwithoutCI` approach (each intermediate CSF is treated independently without CI mixing) with single-electron excitations from the reference configurations:

"""

# ╔═╡ c3001010-0000-4000-8000-000000000010
begin
name          = "Be-like Xe: J=1⁺ and J=1⁻ Green channels for 2-photon decay"
...
wa = Representation(name, Nuclear.Model(54.), Radial.Grid(true), refConfigs,
                    GreenExpansion( AtomicState.SingleCSFwithoutCI(),
                                    Basics.DeExciteSingleElectron(),
                                    [LevelSymmetry(1, Basics.plus), LevelSymmetry(1, Basics.minus)],
                                    4, greenSettings ))
end

# ╔═╡ c3001011-0000-4000-8000-000000000011
md"""

As usual, the representation is generated by `generate()` with `output=true`. This constructs all intermediate levels up to the maximum principal quantum number specified in `GreenSettings` (here $n_\text{max} = 5$) for both symmetries:

"""

# ╔═╡ c3001012-0000-4000-8000-000000000012
wb = generate(wa, output=true)

# ╔═╡ c3001013-0000-4000-8000-000000000013
md"""

The result is stored under the key `"Green channels"`, which returns a list of `GreenChannel` objects — one per requested $J^\pi$ symmetry:

"""

# ╔═╡ c3001014-0000-4000-8000-000000000014
wc = wb["Green channels"]

# ╔═╡ c3001015-0000-4000-8000-000000000015
md"""

Each `GreenChannel` contains a multiplet of intermediate levels for a given symmetry. These channels can then be passed to a two-photon or higher-order process module which performs the sum-over-states using the Green function representation.

## Outlook: two-photon decay rates

The Green channels above provide the intermediate-state sum required for computing the **E1-M1 two-photon decay rate** of the $^3P_0$ level. In the two-photon amplitude, the decay width is proportional to
$$A^{(2)} \propto \sum_n \frac{\langle f \| T^{(E1)} \| n \rangle \langle n \| T^{(M1)} \| i \rangle}{\omega_n - \omega_1},$$
where the sum runs over all intermediate levels $n$ in the $J^\pi = 1^\pm$ channels. The `MultiPhotonDeExcitation` module in JenaAtomicCalculator uses the Green channels directly to evaluate this sum. This computation will be demonstrated in a separate notebook once the interface has been fully consolidated.
