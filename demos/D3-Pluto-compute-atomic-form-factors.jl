...
# ╔═╡ d3001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ d3001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ d3001003-0000-4000-8000-000000000003
md"""
# Atomic form factors for chlorine-like iron and krypton ions
"""

# ╔═╡ d3001004-0000-4000-8000-000000000004
md"""

The **atomic form factor** for elastic x-ray scattering is defined as the Fourier transform of the atomic charge distribution:
$$f(\mathbf{q}) = \int \rho(\mathbf{r})\, e^{i \mathbf{q} \cdot \mathbf{r}}\, d^3r,$$
where $\mathbf{q}$ is the momentum transfer from the photon to the atom. For a spherically symmetric charge distribution, this reduces to a function of the scalar $q = |\mathbf{q}|$. In the forward direction ($q = 0$) the form factor equals the total number of electrons; for large $q$ it decreases as the wavelength of the probe becomes shorter than the orbital radius.

Form factors are widely used as fast approximations to scattering amplitudes and appear in x-ray diffraction, Rayleigh scattering, and photo-ionization cross-section modelling. JenaAtomicCalculator computes them for individual atomic levels via the `FormFactor` module.

"""

# ╔═╡ d3001005-0000-4000-8000-000000000005
? FormFactor.Settings

# ╔═╡ d3001006-0000-4000-8000-000000000006
md"""

The default settings show that form factors are computed for a list of momentum-transfer values $q$ (in atomic units). We can inspect the defaults:

"""

# ╔═╡ d3001007-0000-4000-8000-000000000007
FormFactor.Settings()

# ╔═╡ d3001008-0000-4000-8000-000000000008
md"""

## Chlorine-like iron ($Z = 26$): $[Ne]\,3s^2\,3p^5$

We compute form factors for three momentum-transfer values $q = \{0,\, 0.1,\, 1.0\}$ a.u. and request an overview table. The `FormFactor.Settings` object is passed as an element of `propertySettings` inside an `Atomic.Computation`:

"""

# ╔═╡ d3001009-0000-4000-8000-000000000009
begin
fSettings = FormFactor.Settings([0., 0.1, 1.0], true, LevelSelection())
wa = Atomic.Computation(Atomic.Computation();
                         name            = "Form factors: Cl-like Fe",
...
# ╔═╡ d3001010-0000-4000-8000-000000000010
md"""

The output table lists the form factor $f(q)$ for each level and each requested $q$ value. At $q = 0$, the form factor equals the number of electrons (17 for Cl-like Fe), as expected. With increasing $q$ the value decreases, reflecting the finite spatial extent of the charge distribution.

## Chlorine-like krypton ($Z = 36$)

We repeat the computation for the heavier isoelectronic partner Cl-like Kr simply by updating the nuclear model:

"""

# ╔═╡ d3001011-0000-4000-8000-000000000011
perform(Atomic.Computation(wa; name="Form factors: Cl-like Kr", nuclearModel=Nuclear.Model(36.)))

# ╔═╡ d3001012-0000-4000-8000-000000000012
md"""

Comparing the two tables, the form factor at $q = 1.0$ a.u. is *larger* for Cl-like Kr than for Cl-like Fe. This reflects the relativistic contraction of the inner-shell orbitals with increasing $Z$: the electron density is more concentrated near the nucleus, which means the charge distribution extends less far in space, and the form factor falls off more slowly with $q$.

## Extension: elastic and inelastic scattering cross sections

The form factor is the basic building block for the **elastic differential cross section** for x-ray scattering:
$$\frac{d\sigma}{d\Omega} = r_0^2\, \frac{1 + \cos^2\theta}{2}\, |f(q)|^2,$$
where $r_0$ is the classical electron radius and $\theta$ is the scattering angle. The calculation of elastic and inelastic (Compton) scattering cross sections using the form factors above will be demonstrated in a subsequent tutorial.
