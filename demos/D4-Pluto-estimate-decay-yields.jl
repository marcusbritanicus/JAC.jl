...
# ╔═╡ d4001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ d4001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ d4001003-0000-4000-8000-000000000003
md"""
# Compute fluorescence and Auger yields for K-shell vacancies
"""

# ╔═╡ d4001004-0000-4000-8000-000000000004
md"""
When a K-shell vacancy is created (e.g.\ by photoionization), it decays either radiatively
(fluorescence, emitting a photon) or non-radiatively (Auger emission).  The **fluorescence
yield** $\omega_R$ and the **Auger yield** $\omega_A$ partition the total decay probability:

$$\omega_R \;+\; \omega_A \;\approx\; 1$$

For light elements ($Z \lesssim 20$) Auger emission dominates; for heavier elements
the radiative channel grows rapidly with $Z^4$.

In JAC the `DecayYield` module computes these yields using the statistical Compton
approximation (SCA) or an average-SCA scheme.  Both are controlled by a
`DecayYield.Settings` object passed as a property to `Atomic.Computation`.

A dedicated radial grid with smaller step `h` and a smaller box radius `rbox` is typically
chosen for hole-state calculations, since the K-hole levels are tightly bound.
"""

# ╔═╡ d4001005-0000-4000-8000-000000000005
##  Compact grid suitable for K-hole (tightly-bound) states
grid = Radial.Grid(Radial.Grid(false), h=0.04, rbox=10.0)

# ╔═╡ d4001006-0000-4000-8000-000000000006
md"""
## K-shell vacancy in a 9-electron system at $Z = 10$ (neon)

We model the K-hole state of neutral neon as the 9-electron configuration
`1s 2s² 2p⁶`.  A single `DecayYield.Settings` object in `propertySettings`
triggers the yield computation after the SCF/CI step.
"""

# ╔═╡ d4001007-0000-4000-8000-000000000007
##  Ne K-hole (Z=10, 9 electrons)
ySettings = DecayYield.Settings("SCA", true, false, LevelSelection())

wa_Ne = Atomic.Computation(Atomic.Computation();
             name         = "K-hole decay yields: 9-electron Ne (Z=10)",
             grid         = grid,
             nuclearModel = Nuclear.Model(10.),
             configs      = [Configuration("1s 2s^2 2p^6")],
             propertySettings = [ySettings])
perform(wa_Ne)

# ╔═╡ d4001008-0000-4000-8000-000000000008
md"""
## K-shell vacancy in a 9-electron system at $Z = 12$ (magnesium-like)

Repeating the same 9-electron K-hole calculation at $Z = 12$ (Mg³⁺ with one K-hole)
illustrates how $\omega_R$ grows with nuclear charge.
"""

# ╔═╡ d4001009-0000-4000-8000-000000000009
##  Mg K-hole (Z=12, same 9-electron configuration)
wa_Mg = Atomic.Computation(wa_Ne;
             name         = "K-hole decay yields: 9-electron Mg (Z=12)",
             nuclearModel = Nuclear.Model(12.))
perform(wa_Mg)

# ╔═╡ d4001010-0000-4000-8000-000000000010
md"""
## K-shell vacancy in a 9-electron system at $Z = 18$ (argon-like)

At $Z = 18$ (Ar⁹⁺ with K-hole, 9 electrons) the fluorescence yield is noticeably
larger, demonstrating the $Z^4$ scaling of radiative rates relative to Auger rates.
"""

# ╔═╡ d4001011-0000-4000-8000-000000000011
##  Ar K-hole (Z=18, same 9-electron configuration)
wa_Ar = Atomic.Computation(wa_Ne;
             name         = "K-hole decay yields: 9-electron Ar (Z=18)",
             nuclearModel = Nuclear.Model(18.))
perform(wa_Ar)

# ╔═╡ d4001012-0000-4000-8000-000000000012
md"""
## Summary

The three calculations above confirm the well-known trend: the fluorescence yield
$\omega_R$ increases rapidly with $Z$ while the Auger yield $\omega_A = 1 - \omega_R$
decreases.  For neon ($Z=10$) the decay is almost entirely via Auger emission; by
argon ($Z=18$) the radiative channel already contributes a measurable fraction.

**Key settings used:**
- `DecayYield.Settings("SCA", true, false, LevelSelection())` selects the
  single-configuration approximation (SCA) approach.
- A compact grid `Radial.Grid(Radial.Grid(false), h=0.04, rbox=10.0)` is
  appropriate for tightly-bound K-hole states.
- The `Atomic.Computation` keyword copy-constructor `Atomic.Computation(wa_Ne; nuclearModel=...)` 
  conveniently reuses all other settings.
"""
