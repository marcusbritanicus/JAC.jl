...
# ╔═╡ b7001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ b7001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ b7001003-0000-4000-8000-000000000003
md"""
# Estimate radiative QED corrections for atomic levels
"""

# ╔═╡ b7001004-0000-4000-8000-000000000004
md"""

Most advanced many-electron methods in atomic structure theory are based on the *no-pair* Hamiltonian, which treats a fixed number of electrons but ignores (virtual) photon exchange and electron–positron pair creation. These additional **radiative** or **Lamb-shift corrections** arise from quantum electrodynamics (QED) and must be accounted for separately, especially for inner-shell electrons of heavy atoms.

For hydrogen-like uranium, the QED contribution to the 1s binding energy is roughly 1 % of the total energy — far too large to ignore in precision spectroscopy. For many-electron atoms, the self-energy (photon loop) and vacuum-polarization (pair loop) contributions are typically estimated by an effective operator acting on the one-electron level. JenaAtomicCalculator currently implements two QED models:

   •  `QedPetersburg()` — the Sankt Petersburg model (self-energy + vacuum polarization estimate)
   •  `NoneQed()` — no QED correction (default)

(A `QedSydney()` model is defined but not yet fully implemented.) QED is activated through the `qedModel` field of `AsfSettings`.

"""

# ╔═╡ b7001005-0000-4000-8000-000000000005
? AbstractQedModel

# ╔═╡ b7001006-0000-4000-8000-000000000006
md"""

## Carbon-like xenon ($Z = 54$): $\;1s^2 2s^2 2p^2$ ground configuration

As a first example we compute the fine-structure levels of the $1s^2 2s^2 2p^2$ ground configuration of C-like Xe using the Petersburg QED model. The settings are constructed from the defaults via the keyword copy-constructor:

"""

# ╔═╡ b7001007-0000-4000-8000-000000000007
asfSettings = AsfSettings(AsfSettings(); generateScf=true, maxIterationsScf=20, qedModel=QedPetersburg())

# ╔═╡ b7001008-0000-4000-8000-000000000008
begin
wa_Xe = Atomic.Computation(Atomic.Computation();
                             name         = "QED estimates: C-like Xe",
...
# ╔═╡ b7001009-0000-4000-8000-000000000009
md"""

The output shows the five fine-structure levels $^3P_0$, $^3P_1$, $^3P_2$, $^1D_2$, $^1S_0$ together with QED corrections included in the total energy. For heavier atoms, these corrections grow rapidly and may reach tens of eV for the innermost subshells.

## Beryllium-like uranium ($Z = 92$): with and without QED

As a more extreme example we study the low-lying levels of Be-like uranium arising from the $1s^2 2s^2$ and $1s^2 2s\,2p$ configurations. We first compute **with** the Petersburg QED model and store the result:

"""

# ╔═╡ b7001010-0000-4000-8000-000000000010
begin
wa_U = Atomic.Computation(Atomic.Computation();
                            name         = "QED estimates: Be-like U",
                            grid         = Radial.Grid(true),
                            nuclearModel = Nuclear.Model(92.),
                            configs      = [Configuration("1s^2 2s^2"), Configuration("1s^2 2s 2p")],
                            asfSettings  = asfSettings)
wb_U = perform(wa_U; output=true)
end

# ╔═╡ b7001011-0000-4000-8000-000000000011
md"""

Now we repeat the computation **without** QED (`NoneQed()`) to isolate the QED contribution to the binding energies:

"""

# ╔═╡ b7001012-0000-4000-8000-000000000012
begin
asfNoQed  = AsfSettings(asfSettings; qedModel=NoneQed())
wa_U_noqed = Atomic.Computation(wa_U; asfSettings=asfNoQed)
wb_U_noqed = perform(wa_U_noqed; output=true)
end

# ╔═╡ b7001013-0000-4000-8000-000000000013
md"""

With both sets of levels in hand, we can compare the energies and extract the QED shifts:

"""

# ╔═╡ b7001014-0000-4000-8000-000000000014
begin
levels_qed   = wb_U["multiplet:"].levels
levels_noqed = wb_U_noqed["multiplet:"].levels
nlevels = min(length(levels_qed), length(levels_noqed))
println("  Level   E_QED (au)          E_noQED (au)        ΔE_QED (au)         ΔE_QED (eV)")
println("  " * "-"^85)
for i = 1:nlevels
    dE   = levels_qed[i].energy - levels_noqed[i].energy
    dEeV = Defaults.convertUnits("energy: from atomic to eV", dE)
    sa   = "  " * lpad(i, 3) * "     " *
           lpad(round(levels_qed[i].energy;  digits=6), 18) * "    " *
           lpad(round(levels_noqed[i].energy; digits=6), 18) * "    " *
           lpad(round(dE;   digits=6), 18) * "    " *
           lpad(round(dEeV; digits=2), 10)
    println(sa)
end
end

# ╔═╡ b7001015-0000-4000-8000-000000000015
md"""

The table shows that the QED shift for each level is a negative (binding) correction of several hundred eV for Be-like uranium, reflecting the very large self-energy of the 1s electrons at $Z = 92$. The *differential* shift between levels is smaller but still significant for precision spectroscopy.
