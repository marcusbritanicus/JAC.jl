...
# ╔═╡ c2001001-0000-4000-8000-000000000001
using JenaAtomicCalculator

# ╔═╡ c2001002-0000-4000-8000-000000000002
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ c2001003-0000-4000-8000-000000000003
md"""
# Mean-field basis and CI representation for the oxygen ground configuration
"""

# ╔═╡ c2001004-0000-4000-8000-000000000004
md"""

Various *ab-initio* many-electron methods have been developed to describe the level structure and properties of atoms and ions: configuration interaction (CI), many-body perturbation theory (MBPT), coupled-cluster (CC) theory, correlation potential (CP), and multiconfigurational Dirac–Hartree–Fock (MCDF) are all typically based on some **self-consistent field** (SCF). In JenaAtomicCalculator, different representations of the electronic structure are unified under the `AtomicState.Representation` framework. Each representation combines a nuclear model, a set of reference configurations, a radial grid, and a *representation type* that specifies both the method and its control parameters.

"""

# ╔═╡ c2001005-0000-4000-8000-000000000005
? AtomicState.Representation

# ╔═╡ c2001006-0000-4000-8000-000000000006
md"""

The `repType::AbstractRepresentationType` selects the method. At present, the available types include:

"""

# ╔═╡ c2001007-0000-4000-8000-000000000007
? AtomicState.AbstractRepresentationType

# ╔═╡ c2001008-0000-4000-8000-000000000008
md"""

## Mean-field basis for the oxygen ground configuration

The simplest representation is the **mean-field basis**, controlled by `MeanFieldSettings`. It generates a self-consistent set of orbitals (a Hartree–Fock-like mean field) for a given set of reference configurations:

"""

# ╔═╡ c2001009-0000-4000-8000-000000000009
? MeanFieldBasis

# ╔═╡ c2001010-0000-4000-8000-000000000010
begin
name       = "Oxygen 1s² 2s² 2p⁴ ground configuration"
...
# ╔═╡ c2001011-0000-4000-8000-000000000011
md"""

In JenaAtomicCalculator, an atomic representation is always *generated* by calling `generate()`. Passing `output=true` returns a `Dict{String,Any}` that provides programmatic access to the result:

"""

# ╔═╡ c2001012-0000-4000-8000-000000000012
wb = generate(wa, output=true)

# ╔═╡ c2001013-0000-4000-8000-000000000013
md"""

The dictionary contains the key `"mean-field basis"` which holds the generated basis. We extract the orbital set for use in subsequent calculations:

"""

# ╔═╡ c2001014-0000-4000-8000-000000000014
orbitals = wb["mean-field basis"].orbitals

# ╔═╡ c2001015-0000-4000-8000-000000000015
md"""

## CI representation for the oxygen ground configuration

We can now use this mean-field orbital basis to generate a **configuration-interaction (CI) representation** for all levels of the $1s^2 2s^2 2p^4$ ground configuration. The excitation scheme (no additional excitations here) is specified via `RasStep`, and the interaction type and level selection via `CiSettings`:

"""

# ╔═╡ c2001016-0000-4000-8000-000000000016
? CiSettings

# ╔═╡ c2001017-0000-4000-8000-000000000017
begin
...
# ╔═╡ c2001018-0000-4000-8000-000000000018
wd = generate(wc, output=true)

# ╔═╡ c2001019-0000-4000-8000-000000000019
md"""

The output dictionary now contains the key `"CI multiplet"` with the requested levels:

"""

# ╔═╡ c2001020-0000-4000-8000-000000000020
levels = wd["CI multiplet"].levels

# ╔═╡ c2001021-0000-4000-8000-000000000021
md"""

The `levels` array holds a `Level` object for each fine-structure state of the $1s^2 2s^2 2p^4$ configuration. Each level carries its energy, total $J$ value, parity, and the expansion coefficients over the CI basis. These objects can be passed directly to property or process modules for subsequent calculations — for instance, to compute transition rates or hyperfine constants, as shown in other tutorials.

## Connection to the RAS approach

The workflow above — mean-field basis $\to$ CI expansion $\to$ multiplet — is the building block for the **restricted-active-space** (RAS) method, where systematic electron excitations from the reference configurations into progressively larger active spaces allow convergence to the full CI limit. The `CiExpansion` with a non-trivial `RasStep` provides this control. See `C1-Pluto-compute-RAS-carbon-III.jl` for a worked RAS example.
