...
using Printf, JAC, ..Basics, ..Defaults, ..ManyElectron, ..Nuclear, ..Radial, ..TableStrings
...
function computeOutcomes(initialConfigs::Array{Configuration,1}, asfSettings::AsfSettings, 
    ...
    wa = JAC.Cascade.Computation(JAC.Cascade.Computation(), name="photon lines", nuclearModel=nm, grid=grid, asfSettings=asfSettings, 
                                scheme=JAC.Cascade.StepwiseDecayScheme([Radiative()], 0, Dict{Int64,Float64}(), 0, decayShells, Shell[], Shell[]),
    ...
    wa = JAC.Cascade.Computation(JAC.Cascade.Computation(), name="Auger lines", nuclearModel=nm, grid=grid, asfSettings=asfSettings, 
                                scheme=JAC.Cascade.StepwiseDecayScheme([Auger()], 1, Dict{Int64,Float64}(), 0, decayShells, Shell[], Shell[]),
                                approach=cApproach, initialConfigs=initialConfigs)
    ...
    for  outcome in outcomes
        ...
        #
        if settings.geant4
            JAC.Cascade.computeDecayProbabilities(newOutcome, linesR, linesA, settings)
        end
    end
    #
