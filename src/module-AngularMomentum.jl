...
function Wigner_3j(a, b, c, m_a, m_b, m_c)
    try
        return WignerSymbols.wigner3j(Basics.twice(a)/2.0,   Basics.twice(b)/2.0,   Basics.twice(c)/2.0,
            Basics.twice(m_a)/2.0, Basics.twice(m_b)/2.0, Basics.twice(m_c)/2.0)
    catch e
        return 0.0 # Gracefully turns unphysical combinations to zero
    end
end
...
function Wigner_6j(a, b, c, d, e, f)
    try
        return WignerSymbols.wigner6j(Basics.twice(a)/2.0, Basics.twice(b)/2.0, Basics.twice(c)/2.0,
